...
Download the repository and unzip the `test.zip` in the same folder.

Install MAFFT and set the installation path in `config.txt`, for example `align_fastq_with_mafft.my_mafft <- "D:/R/mafft-win/mafft.bat"`

Run the `denoise.R` script.

Consult the `results.fasta` inside the test folder for clusters generated by the pipeline.
