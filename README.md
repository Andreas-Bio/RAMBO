...
7.	**Consensus Merging** : The pipeline includes a post-processing step to merge very similar consensus sequences within each sample, which helps if the algorithm accidentally over-split a true sequence into two clusters (for instance, due to a borderline difference or a few errors). This step takes all consensus sequences from a sample’s `_final.fasta` and compares them pairwise:
